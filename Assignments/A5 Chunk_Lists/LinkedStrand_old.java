    ...
    */
    public DnaStrand cutAndSplice(String enzyme, String splice) {
        ...
        while (p.dnaInfo.contains(enzyme)) {
            ...
            p.next = new Node(ret.rear.dnaInfo.substring(pos + enzyme.length()), null);
            ret.rear = p.next;
            if (pos != 0) {
                p.next = new Node(p.dnaInfo.substring(pos), ret.rear);
                p.next.dnaInfo = splice;
